...
// functions/tools/utils.js
var argMin = (arr, func) => arr.map((elem, idx) => [func ? func(elem) : elem, idx]).reduce((acc, curr) => acc[0] < curr[0] ? acc : curr)[1];
var validCoordinates = (lat, lon) => {
  const packageReturn = (valid, body2) => ({ valid, body: body2 });
  if (!(lat && lon)) {
    const body2 = {
      message: "Must include both the latitude (lat) and longitude (lon) in the query parameters"
    };
    return packageReturn(false, body2);
  }
...
var openskyLatLonString = (lat, lon, searchRadius) => `lamin=${Math.max(lat - searchRadius, -90)}&lomin=${Math.max(lon - searchRadius, -180)}&lamax=${Math.min(lat + searchRadius, 90)}&lomax=${Math.min(lon + searchRadius, 180)}`;

// functions/airplanes.ts
var API_ENDPOINT = "https://opensky-network.org/api";
var handler = async (event, _) => {
  ...
  return fetch(`${API_ENDPOINT}/states/all?${openskyLatLonString(lat, lon, range)}`).then((response) => response.json()).then((data) => {
